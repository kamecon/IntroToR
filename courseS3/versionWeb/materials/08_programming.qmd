...
::: {.callout-tip title="Example: Sampling distribution of a linear regression coefficients"}
...
Compute the test score and append it to `CASchools`
...
CASchools[sample(nrow(CASchools),100), ]

```


We fill the vector with the estimated betas by taking only a sample of the original data
...
Instead of using a loop you can use the `replicate` function, one of the `apply` family of functions, which apply a function to a vector of values, which is the same thing a loop does when it _applies_ repeatedly the same operation to a set of values represented by a vector. In this case we are doing something simpler, repeating a number of times an instruction encapsulated in a function, in this case repeatedly extracting a sample from a population and calculating a regression. So `replicate` in our example repeats 1000 times the operation described above.
This type of operations is often performed in statistics, since in that way we do _simulations_, repeat many times realizations of a probability distribution or a phenomenon whose outcome is uncertain (roll a die 50,000 times)
