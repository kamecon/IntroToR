...
<!-- And to visualize the effects, we use the function `cplot` -->

<!-- ```{r marginal4} -->

<!-- cplot(model_2, x = "pirat", what = "effect", data = HMDA) -->

<!-- ``` -->

## Differences in differences

Without entering in theoretical details aboy desing and others, we will introduce the `feols()` function from the [`fixest`](https://lrberge.github.io/fixest/index.html) library.

This library _offers a family of functions to perform estimations with multiple fixed-effects in both an OLS and a GLM context_. It also supports efficient instrumental variables (IV) estimation, as well as a wide range of GLM models like logit, probit, Poisson, and negative binomial

Given that this library handle well fixed effect estimation, it's well suited for diff in diff analysis

Using the organ donors case treated in [Kessler and Roth (2014)](https://www.nber.org/papers/w20378) paper
...
The treated group is the state of California, and the treatment period the 3 quarters from July 2011 to March 2012

We create the treatment variable
...
The `feols` function  automatically clusters SEs by the first fixed effect (if there are any)
...
We use the `modelsummary` library to elaborate a user friendly regression table
