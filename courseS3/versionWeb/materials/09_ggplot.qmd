...
We will introduces the [`sf` library](https://r-spatial.github.io/sf/)

We are going download data in sf format from Eurostat using the `eurostat` library
...
We download data from eurostat
...
Let us plot the data using the geometry `geom_sf` and see what we have
...
As we mention in the previous example, we would link the geographical location with some numerical data. We will download unemployment data at NUTS2 level
...
We will filter the unemployment rate for all gender and education level for 2021
...
We join the unmeployment data with the geolocation data only for Italy
