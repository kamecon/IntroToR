...
pacman::p_load(openxlsx, knitr, tibble, readxlsb, dplyr)
...
## Environmental extensions (CO2 emissions)

::: {#fig-mrio}
![](images/MRIO.jpg)
Multi-country input-output tables with environmental extensions
:::


### Read the data

We have our input-output table in csv format. We load it as an R object in memory.

Notice that we will upload the **full table**

```{r mrio01}

tableData <- read.csv(file = "PartII/example.csv")
...
rownames(tableData) <- tableData$X

tableData$X <- NULL

tableData[c("B1G", "P1", "C02"), c("Demand.Country.A",  "Demand.Country.B", "Demand.Country.C", "P1")] <- 0
...
matrixData <- as.matrix(tableData)

matrixData

```

We extract the $Z$ matrix

```{r mrio22}
#| output: false

Z <- matrixData[1:9,1:9]
...
We extract the emissions vector 

```{r mrio6}

co2 <- matrixData["C02", 1:9]

co2
```


We obtain the emissions intensities (per unit of output) or direct impact coefficients
...
That is:
$$\frac{\text{emissions}}{x} = g \Rightarrow \text{emissions} = gx =  g(I-A)^{-1}y$$
$$\text{emissions} = g(I-A)^{-1}y$$
$$\Delta \text{emissions} = g(I-A)^{-1} \Delta y$$

$g(I-A)^{-1}$ is the *emissions multiplier* $M$

In matrix form:

$$M = \hat{g}(I-A)^{-1}$$
where $\hat{g}$ is a diagonal matrix with the elements of $g$ in the main diagonal

$M$ tell us how the emissions change in response of a unit change in the final demand

<!-- $$M = \hat{g}(I-A)^{-1} = \hat{g} \frac{\Delta x}{\Delta y} = \hat{g}B$$ -->
...
The element $M_{1,1}$ indicates that in order to meet a 1 unit increase in demand for industry 1 products, `r M[1, 1]` units of industry 1 emissions are generated.

Where does this figure comes from? The element $B_{1,1}$ indicates that in order to meet a 1 unit increase in demand for industry 1 products, `r B[1, 1]` units of industry 1 input are required. This number times `r g[1]` results in `r M[1, 1]`

Similarly, the element $M_{2,1}$ shows that to satisfy a 1 unit increase in demand industry 1 products, `r M[2, 1]` units of emissions in industry 2 are generated as consequence of the higher output in that sector needed to satisfy the higher demand.

As before, by calculating the column sums of matrix $M$, we can determine the *emissions multiplier* for industry $j$, which represents the change in emissions by industry associated to a unit change in demand


```{r mrio9}

m <- colSums(M, na.rm = TRUE)
...
To obtain the total pollution impact generated per unit of product $j$ for final use in the economy, we need the final demand matrix

```{r mrio10}

nombresColumnas <- dimnames(matrixData)[[2]]

demand <- grepl(x = nombresColumnas, pattern = "Demand")

demandMatrix <- matrixData[1:9, demand]

```

The carbon footprint is the level of emissions associated to a given matrix of final demand $y$

From the above expression

$$\text{emissions} = \hat{g}(I-A)^{-1}y$$
$$\text{emissions} = My$$

```{r mrio11}

results <- M%*%demandMatrix

rownames(results) <- rownames(demandMatrix)
...
carbonFootprint <- colSums(results)
